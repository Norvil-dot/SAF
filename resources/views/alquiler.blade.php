...
@section('title', 'Locales en Alquiler')
...
<div class="bg-white dark:bg-darkcard border-b border-slate-200 dark:border-slate-800 sticky top-16 z-30 shadow-sm transition-colors duration-200">
    <div class="container mx-auto px-4 py-4 flex flex-wrap gap-3 items-center">
        ...
        <select onchange="window.location.href=this.value" class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-lg px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-accent shadow-sm cursor-pointer font-bold border-l-4 border-l-accent">
            <option value="{{ route('venta') }}">Venta</option>
            <option value="{{ route('alquiler') }}" selected>Alquiler</option>
        </select>
        ...
        <div x-data="{ openFilters: false }">
            ...
            <div x-show="openFilters" style="display: none;" class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center">
                <form action="{{ route('alquiler') }}" method="GET" @click.away="openFilters = false" class="bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
...
<div class="container mx-auto px-4 py-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 border-b border-slate-200 dark:border-slate-800 pb-4 gap-4">
        <div>
            <div class="flex items-center gap-2 mb-1">
                <span class="bg-accent/10 text-accent text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-md">Alquiler</span>
                <span class="text-sm text-slate-500 dark:text-slate-400 font-medium">Resultados para empresas</span>
            </div>
            <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Locales en Alquiler</h1>
        </div>
        @php
            $sortMap = [
    ...
    <div class="flex gap-4 mb-8 text-sm overflow-x-auto pb-2 whitespace-nowrap hide-scrollbar">
        ...
        @endphp
        <a href="?ubicacion={{ urlencode($ubicacionConteo->distrito) }}" class="bg-white dark:bg-darkcard border {{ $isActive ? 'border-accent dark:border-accent shadow-md ring-1 ring-accent/20' : 'border-slate-200 dark:border-slate-800 shadow-sm' }} hover:border-accent/50 dark:hover:border-accent/50 rounded-lg px-6 py-3 flex items-center gap-3 transition-all cursor-pointer">
            <div class="w-2 h-2 rounded-full {{ $isActive ? 'bg-accent' : 'bg-slate-300 dark:bg-slate-600' }}"></div>
            <div>
    ...
    <div class="space-y-6 max-w-5xl">
        ...
        <div class="group bg-white dark:bg-darkcard border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col md:flex-row shadow-sm hover:shadow-xl hover:border-primary-200 dark:hover:border-primary-900/50 transition-all duration-300 overflow-hidden">
            ...
            <div class="p-6 md:w-3/5 flex flex-col justify-between">
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-primary-600 dark:text-primary-400 my-2">S/ {{ number_format($local->precio_mensual, 2) }} <span class="text-sm font-medium text-slate-400">/mes</span></p>
        ...
        <div class="w-full bg-white dark:bg-darkcard border-2 border-dashed border-slate-300 dark:border-slate-700/60 rounded-[2rem] p-12 md:p-16 flex flex-col items-center justify-center text-center transition-all hover:border-primary-400 dark:hover:border-primary-500/50 shadow-sm col-span-full my-8">
            ...
            @elseif(request()->has('min_precio') || request()->has('max_precio'))
                ...
                <p class="text-base text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-8 leading-relaxed">No hemos encontrado propiedades con los precios especificados. Es posible que ajustando el rango mínimo y máximo encuentres excelentes opciones para alquilar.</p>
            @elseif(count(request()->all()) > 0 && !request()->has('sort'))
                ...
                <p class="text-base text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-8 leading-relaxed">No logramos encontrar propiedades en alquiler que cumplan con todos los filtros que has aplicado. Intenta relajar algunas condiciones de tu búsqueda.</p>
            @else
                <h3 class="text-2xl font-extrabold text-slate-800 dark:text-white mb-3 tracking-tight">Aún no hay propiedades en alquiler</h3>
                <p class="text-base text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-8 leading-relaxed">Por el momento no tenemos inmuebles publicados para alquilar en esta sección. Vuelve pronto para descubrir nuevas opciones.</p>
            @endif

            @if(count(request()->all()) > 0)
            <div class="mt-2">
                <a href="{{ route('alquiler') }}" class="inline-flex items-center gap-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 px-6 py-3.5 rounded-xl font-bold transition-all shadow-sm hover:shadow-md">
